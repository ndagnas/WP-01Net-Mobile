...
//*************************************************************************************************
// Nom           : InflateManager.cs
// Auteur        : Nicolas Dagnas
// Description   : Implémentation de l'objet InflateManager
        ...
        Label_001D:
            switch (this.mode)
            {
                case InflateManagerMode.METHOD:
                    if (this._codec.AvailableBytesIn != 0)
                    {
                        ...
                        if (((this.method = this._codec.InputBuffer[this._codec.NextIn++]) & 15) != 8)
                        {
                            ...
                            this._codec.Message = string.Format("unknown compression method (0x{0:X2})", new object[] { this.method });
                            this.marker = 5;
                        }
                        else if (((this.method >> 4) + 8) > this.wbits)
                        {
                            this.mode = InflateManagerMode.BAD;
                            this._codec.Message = string.Format("invalid window size ({0})", new object[] { (this.method >> 4) + 8 });
                            this.marker = 5;
                        }
                        else
                        {
                ...
                case InflateManagerMode.BAD:
                    throw new ZlibException(string.Format("Bad state ({0})", new object[] { this._codec.Message }));
            }
