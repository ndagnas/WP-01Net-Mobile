...
//*************************************************************************************************
// Nom           : ZlibBaseStream.cs
// Auteur        : Nicolas Dagnas
// Description   : Implémentation de l'objet ZlibBaseStream
...
namespace System.IO.Compression
    ...
    internal class ZlibBaseStream : Stream
    {
        ...
        private int _ReadAndValidateGzipHeader()
        {
            ...
            this._GzipMtime = new DateTime(0x7b2, 1, 1, 0, 0, 0).AddSeconds((double) num3);
            num += num2;
            if ((buffer[3] & 4) == 4)
            {
            ...
            if (this._streamMode != StreamMode.Writer)
            {
                if ((this._streamMode == StreamMode.Reader) && (this._flavor == ZlibStreamFlavor.GZIP))
                {
                    ...
                    if (this._z.AvailableBytesIn != 8)
                    {
                        ...
                        if (count != num5)
                        {
                            throw new ZlibException(string.Format("Protocol error. AvailableBytesIn={0}, expected 8", new object[] { this._z.AvailableBytesIn + num5 }));
                        }
                    }
                    else
                    {
                    ...
                    if (num7 != num6)
                    {
                        throw new ZlibException(string.Format("Bad CRC32 in GZIP stream. (actual({0:X8})!=expected({1:X8}))", new object[] { num7, num6 }));
                    }
                    if (num9 != num8)
                    {
                        throw new ZlibException(string.Format("Bad size in GZIP stream. (actual({0})!=expected({1}))", new object[] { num9, num8 }));
                    }
                }
                return;
            }
        ...
        Label_0016:
            ...
            switch (num)
            {
                ...
                default:
                {
                    ...
                    {
                        throw new ZlibException(string.Format("{0}: (rc = {1})", new object[] { str, num }));
                    }
        ...
        public override int Read(byte[] buffer, int offset, int count)
        {
            ...
            do
            {
                ...
                if ((num != 0) && (num != 1))
                {
                    throw new ZlibException(string.Format("{0}flating:  rc={1}  msg={2}", new object[] { this._wantCompress ? "de" : "in", num, this._z.Message }));
                }
            }
            ...
            if (this._z.AvailableBytesOut > 0)
            {
                ...
                if (this.nomoreinput && this._wantCompress)
                {
                    ...
                    if ((num != 0) && (num != 1))
                    {
                        throw new ZlibException(string.Format("Deflating:  rc={0}  msg={1}", new object[] { num, this._z.Message }));
                    }
                }
            }
