	...
	public class Article
		...
		private static T Value<T> ( JToken Token, string Name, T Default )
			...
			try
				...
				return ( Value != null && Value.Value != null && Value.Value is T ) ? 
		...
		public void Load ( JToken Token )
			...
			//-------------------------------------------------------------------------------------------------------------------
			string ArticleID = Value<long  >( Token, "article", 0            ).ToString ();
			string VideoID   = Value<string>( Token, "video"  , string.Empty );

			if ( ! string.IsNullOrEmpty ( VideoID ) )
				{ this.ArticleID = VideoID;   this.IsVideo = true;  }
			else
				{ this.ArticleID = ArticleID; this.IsVideo = false; }
			//-------------------------------------------------------------------------------------------------------------------
			...
			if ( this.IsVideo )
				{
				//---------------------------------------------------------------------------------------------------------------
				this.PublishDate = Token.Read<DateTime>( "begin_date" );
				this.UpdateDate  = Token.Read<DateTime>( "end_date"   );

				if ( this.UpdateDate <= this.PublishDate ) this.UpdateDate = DateTime.MinValue;

				this.Icon         = Value<string>( Token, "image_small", this.Icon );
				this.SubSection   = this.Section;
				this.SubSectionID = this.SectionID;
				//---------------------------------------------------------------------------------------------------------------
				}
			//-------------------------------------------------------------------------------------------------------------------
		...
		public void Load ( IDictionary<string, string> QueryString )
			...
			// IsVideo
			//-------------------------------------------------------------------------------------------------------------------
			if ( QueryString.ContainsKey ( "IsVideo" ) )
				this.IsVideo = QueryString["IsVideo"].IsTrue ();
			//-------------------------------------------------------------------------------------------------------------------
		...
		public string UrlEncode ( params string[] Names )
			...
			foreach ( string Name in Names )
				...
				switch ( Name )
					...
					case "IsVideo"      :
						Result["IsVideo"     ] = this.IsVideo     .ToString (); break;
					case "SectionID"    :
